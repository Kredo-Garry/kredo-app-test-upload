                    ...
                    @foreach ($all_classes as $class)
                        <div class="col-12 my-2 p-5 shadow">
                            ...
                            @if (Auth::user()->role_id == 3)
                                @if ($class->status == 'vacant')
                                    <form action="{{ route('student.book', $class->id) }}" method="post">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm px-5 btn-outline-success float-end">Book
                                            class</button>
                                    </form>
                                @elseif($class->status == 'booked')
                                    <button type="submit" class=" btn btn-sm px-5 btn-warning float-end " disabled>Class
                                        booked</button>
                                @elseif($class->status == 'ended')
                                    <button type="submit" class=" btn btn-sm px-5 btn-danger float-end " disabled>Class
                                        ended</button>
                                @endif
                            @endif
